...
\texttt{sudo apt install texlive-full}. See \href{https://github.com/philipdarke/simple-thesis/issues/1\#issuecomment-1256169085}{\texttt{https://github.com/philipdarke/simple-thesis/issues/1\#issuecomment-1256169085}}
if not using the full TexLive install.} and MacOS with MacTeX\footnote{MacOS 12 Monterey,
13 Ventura and 14 Sonoma with MacTeX installed using \texttt{brew install --cask mactex-no-gui}}.
If the document fails to build, try \texttt{make purge} to delete all output and intermediate
files\footnote{The \texttt{make clean} command removes intermediate files only.}.
